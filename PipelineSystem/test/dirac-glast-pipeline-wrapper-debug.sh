...
gotEXIT()
{
    ...
    if [ -e ${PIPELINE_SUMMARY} ] ; then
       ...
       if [ "$1" != "rerun" ] ; then
           dirac-glast-pipeline-sendmail -T ${PIPELINE_DEBUG_ADDRESS} -F ${PIPELINE_FROMADDRESS} -S ${PIPELINE_PROCESSINSTANCE} -f "${PIPELINE_SUMMARY}"
           dirac-glast-pipeline-sendmail -T ${PIPELINE_TOADDRESS} -F ${PIPELINE_FROMADDRESS} -S ${PIPELINE_PROCESSINSTANCE} -f "${PIPELINE_SUMMARY}"
           fi
    else
       if [ "$1" != "rerun" ] ; then
           dirac-glast-pipeline-sendmail -T ${PIPELINE_DEBUG_ADDRESS} -F ${PIPELINE_FROMADDRESS} -S ${PIPELINE_PROCESSINSTANCE} -B "${MSG}\n${END}"
